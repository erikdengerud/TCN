...
def evaluate_final() -> List[float]:
    ...
    with torch.no_grad():
        ...
        predictions = torch.cat(all_predictions, axis=0).detach().cpu().numpy()
        print(predictions.shape)
        predictions = train_dataset.inverse_transform(predictions.T).T
        print(predictions.shape)
        predictions_tensor = torch.from_numpy(
            predictions, dtype=torch.float32, device="cpu"
        )

        real_values_tensor = (
            test_dataset_unscaled.X[
                :, 0, -args.num_rolling_periods * args.length_rolling :
            ]
            .squeeze(1)
            .cpu()
        )
...
if __name__ == "__main__":
    ...
    print("Test dataset")
    test_dataset_unscaled = RevenueDataset(
        file_path="revenue/data/processed_companies.csv",
        meta_path="revenue/data/comp_sect_meta.csv",
    ...
    print(test_dataset_unscaled.X)
    test_dataset_scaled = RevenueDataset(
        file_path="revenue/data/processed_companies.csv",
        meta_path="revenue/data/comp_sect_meta.csv",
        start_date=test_start,
        end_date=test_end,
    ...
    length_test = test_dataset_unscaled.length_ts
    print(length_train)
    print(length_test)
