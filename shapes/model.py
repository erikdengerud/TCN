...
from TCN.layers import DilatedCausalConv, ResidualBlock
...
if __name__ == "__main__":
    ...
    dataset = ShapeDataset(
        ...
        predict_ahead=1,
        h_batch=32,
        receptive_field=49,  # 3 layers, kernel size 7
        mean=0,
        var=1,
    )
    ...
    for i, data in enumerate(data_loader):
        ...
        print(y.shape)
        print("i=", i)
        out = tcn(x, emb_id=idx_row)
        print("out.shape", out.shape)
        preds, real = tcn.rolling_prediction(x, tau=24, num_windows=1, emb_id=idx_row)
