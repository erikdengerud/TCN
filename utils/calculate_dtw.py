...
X_train_scaled = ds_train_scaled.X.squeeze(1).detach().cpu().numpy().astype("double")
X_train_unscaled = (
    ds_train_unscaled.X.squeeze(1).detach().cpu().numpy().astype("double")
)
X_test_scaled = ds_test_scaled.X.squeeze(1).detach().cpu().numpy().astype("double")
X_test_unscaled = ds_test_unscaled.X.squeeze(1).detach().cpu().numpy().astype("double")
import time
...
S = distance_matrix_fast(X_train_unscaled)
S[np.isinf(S)] = 0
S = S + S.T
